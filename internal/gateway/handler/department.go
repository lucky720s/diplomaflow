...
func (h *Handler) GetDepartmentDashboard(c *gin.Context) {
	...
	resp, err := h.adminClient.GetDashboard(outgoingCtx(c), &adminv1.GetDashboardRequest{
		DepartmentId: deptID,
	})
	if err != nil {
		MapGRPCError(c, err)
...
func (h *Handler) GetDepartmentSubmissions(c *gin.Context) {
	...
	resp, err := h.adminClient.ListSubmissions(outgoingCtx(c), &adminv1.ListSubmissionsRequest{
		DepartmentId: deptID,
		Status:       status,
		Page:         1,
		PageSize:     20,
...
func (h *Handler) GetDepartmentTopicRegistrations(c *gin.Context) {
	...
	resp, err := h.adminClient.ListTopicRegistrations(outgoingCtx(c), &adminv1.ListTopicRegistrationsRequest{
		DepartmentId: deptID,
		Status:       status,
		Page:         1,
		PageSize:     20,
