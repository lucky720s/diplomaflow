...
func (s *Service) CreateProject(ctx context.Context, req *projectv1.CreateProjectRequest) (*projectv1.CreateProjectResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("no active workflow for department: %w", err)
	}
	if len(wf.States) == 0 {
		return nil, errors.New("workflow has no states")
	}

	// initial state
	var initial *workflowv1.State
	...
	if initial == nil {
		initial = wf.States[0]
		for _, st := range wf.States[1:] {
			if st.OrderIndex < initial.OrderIndex {
				initial = st
			}
		}
	}
...
// Legacy endpoint used by frontend, but without any legacy processors
func (s *Service) PerformAction(ctx context.Context, projectID int64, actionName string, payload map[string]interface{}, userID int64, userRole string) (*Project, error) {
	...
	// Forward role to workflow_service via metadata (engine uses role conditions)
	...
	if transitionID == 0 {
		return nil, fmt.Errorf("unknown action/transition for this state: %s", actionName)
	}
	...
	// In real runtime, workflow_service commits via CommitTransition RPC.
	// Here we just return current project state from DB.
...
// CommitTransition: transactional state update + history + outbox post-actions
func (s *Service) CommitTransition(ctx context.Context, req *projectv1.CommitTransitionRequest) (*projectv1.CommitTransitionResponse, error) {
	err := s.repo.Transaction(ctx, func(tx *gorm.DB) error {
		...
		if p.CurrentStateID != req.ExpectedFromStateId {
			return fmt.Errorf("concurrent update: expected state %d, got %d", req.ExpectedFromStateId, p.CurrentStateID)
		}

		if req.DataPatch != nil {
			patch := req.DataPatch.AsMap()
			merged, mergeErr := mergeJSONObjects(p.Data, patch)
			if mergeErr != nil {
				return mergeErr
			}
			p.Data = merged
...
func mergeJSONObjects(current datatypes.JSON, patch map[string]interface{}) (datatypes.JSON, error) {
	...
	for k, v := range src {
		if vMap, ok := v.(map[string]interface{}); ok {
			if existing, ok := dst[k].(map[string]interface{}); ok {
				...
				dst[k] = deepMerge(map[string]interface{}{}, vMap)
			}
		} else {
			dst[k] = v
		}
