...
func main() {
	...
	reg := metrics.NewRegistry("university_service")
	reg.MustRegister(metrics.GRPCCollectors()...)

	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(metrics.UnaryServerInterceptor("university_service")),
	)
	...
	log.Info("University metrics endpoint", zap.String("port", metricsPort))

	// gRPC health
	healthServer := health.NewServer()
	grpc_health_v1.RegisterHealthServer(grpcServer, healthServer)
	healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_SERVING)
	healthServer.SetServingStatus("university.v1.UniversityService", grpc_health_v1.HealthCheckResponse_SERVING)
	...
	healthServer.SetServingStatus("university.v1.UniversityService", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
